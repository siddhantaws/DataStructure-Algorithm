...
public class FiniteAutomata extends PatternMatch {
    ...
    private void constructFiniteAutomataArray() {
        ...
        for (i = 1; i < this.patternlength + 1; ++i) {
            Character currentChar = this.pattern.toCharArray()[i] ;
            ...
            if (i < this.patternlength) {
                this.finiteAutomataArray[i][this.characterToIndexMap.get(currentChar)] = i + 1;
                lps = this.finiteAutomataArray[lps][this.characterToIndexMap.get(currentChar)];
            }
        }
