 ...
 */
public class Find_LCABinary_Tree_Using_RMQ {

    private Node root;
    private int V= 9;
    int euler[] ; // for euler tour sequence
    int level[] ; // level of nodes in tour sequence
    int f_occur[] ; // to store 1st occurance of nodes
    int fill; // variable to fill euler and level arrays
    St_class sc ;

    public Find_LCABinary_Tree_Using_RMQ(Node root) {
        this.root = root;
        this.euler = new int [2 * V - 1]; // for euler tour sequence
        this.level = new int[2 * V - 1]; // level of nodes in tour sequence
        this.f_occur =  new int[2 * V - 1]; // to store 1st occurance of nodes
        this.sc =new St_class();
    }

    int Log2(int x) {// log base 2 of x
        int ans = 0;
        int y = x >>= 1;
        while (y-- != 0)
            ans++;
    ...
    // Return minimum of elements in range from index qs (quey start) to qe (query end).  It mainly uses RMQUtil()
    ...
    void eulerTour(Node node, int l) {// Recursive version of the Euler tour of T
        if (node != null)   {/* if the passed node exists */
            ...
            /* tour left subtree if exists, and remark euler and level arrays for parent on return */
            if (node.left != null) {
            ...
            /* tour right subtree if exists, and remark euler and level arrays for parent on return */
            if (node.right != null) {
    ...
    int findLCA(Node node, int u, int v) {// returns LCA of node n1 and n2 assuming they are present in tree
        /* Mark all nodes unvisited.  Note that the size of firstOccurrence is 1 as node values which vary from
        ...
        /* If v before u in Euler tour.  For RMQ to work, first parameter 'u' must be smaller than second 'v' */
