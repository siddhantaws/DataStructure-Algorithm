 ...
 */
public class SubtractTwoNumbersRepresentedLinkedLists {
    ...
    public SubtractTwoNumbersRepresentedLinkedLists(Node node1, Node node2) {
        this.node1 = node1;
        this.node2 = node2;
    }
    ...
    private Node paddZeros(Node sNode, int diff) {
        if (sNode == null)
            return null;
    ...
    private Node subtractLinkedList(Node l1, Node l2) {
        ...
        Node temp1 = l1;
        Node temp2 = l2;
        // If lengths differ, calculate the smaller Node and padd zeros for smaller Node and
        // ensure both larger Node and smaller Node has equal length.
        if (len1 != len2) {
    ...
    /* Subtract LinkedList Helper is a recursive function, move till the last Node, and subtract the digits and create the Node and
    return the Node. If d1 < d2, we borrow the number from previous digit. */
    private Node subtractLinkedListHelper(Node l1, Node l2) {
